...
def get_loader(transform,
    ...
    img_folder = os.path.join(cocoapi_loc, f'cocoapi/images/{img_folder}/')
    annotations_file = os.path.join(cocoapi_loc, f'cocoapi/annotations/{annotations_file}')
...
class CoCoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # obtain image if in test mode
            ...
            captions = [ann["caption"] for ann in self.coco.imgToAnns[img_id]]
            # path = self.paths[index]

            # Convert image to tensor and pre-process using transform
            PIL_image = Image.open(os.path.join(
                self.img_folder, img_path)).convert('RGB')
